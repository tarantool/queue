...
## `stube` - очередь с раздельными очередями

Особенности.
Работает аналогично `fifo`. В этой очереди так же нет `ttl`, `ttr` итп.
Отличие в том, что каждый `put` и `take` принимает в опциях параметр `stube` - имя
микроочереди. И нужно вытаскивать данные только по ключу, при этом нет блокировки
на доступ к следующей задаче, как в `utube`.
