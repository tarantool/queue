...
local signed_num_type = qc.signed_num_type
...
function tube.create_space(space_name, opts)
    ...
    space:create_index('status', {
        type          = 'tree',
        parts         = {i_status, str_type(), i_pri, signed_num_type(), i_id, num_type()},
        if_not_exists = if_not_exists
    })
...
local function fiber_iteration(self, processed)
    local now       = time()
    local task      = nil
...
-- watch fiber
local function lifottl_fiber(self)
    fiber.name('lifottl')
    log.info("Started queue lifottl fiber")
    local processed = 0

    while true do
        if not box.cfg.read_only then
            local stat, err = pcall(fiber_iteration, self, processed)
...
function tube.new(space, on_task_change, opts)
    ...
    self.cond  = qc.waiter()
    self.fiber = fiber.create(lifottl_fiber, self)
...
function method.take(self)
    ...
    while true do
        task = self.space.index.status:select({state.READY},
                {offset=offset, limit=1, iterator='LE'})[1]
